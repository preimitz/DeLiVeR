...
#
####################################################################################
## Parameter set for hadronic current, parametrization taken from arXiv:1610.00235 ##
####################################################################################
#
...
# Decay rate of mediator-> Omega Pion
def GammaDM(mMed):
    Q = mMed
    Q2 = Q**2
    if(Q2>(mOmega_+mPi_)**2 and cI1_ != 0) :
        ...
        return 0.
    return 1/12./math.pi*pcm**3*abs(FOmPiGamma(Q2))**2*brPi0Gamma_
...
def sigmaDM(Q2) :
    ...
    DMmed2 = abs(DMmed)**2
    return DMmed2/12./math.pi*Q*(1+2*mDM_**2/Q2)*pcm**3*abs(FOmPiGamma(Q2))**2*brPi0Gamma_*par.gev2nb

# cross-section for e+e- -> Omega pion
def sigmaSM(Q2) :
    Q = math.sqrt(Q2)
    #print "alpha: ", alpha.alphaEM(Q2), "at ", Q, " GeV"
    if(Q>mOmega_+mPi_) :
        ...
        return 0.
    return 4.*math.pi*alpha.alphaEM(Q2)**2*pcm**3/3./Q/Q2*abs(FOmPiGamma(Q2))**2*brPi0Gamma_*par.gev2nb
